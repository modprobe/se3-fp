...
(define (some p? xs)
  (cond [(= (length xs) 0) #f]
        ...
        [else (some p? (cdr xs))]))
;;;;;
;Wäre jeweils auch bequem über Fktn höherer Ordnung lösbar
;6/6 Punkten
;;;;;
...
(define (transitiv? relation)
  (every
   ...
   relation))
;;;;;
;Die Transitivitätsüberprüfung muss für jedes Paar gegenüber jedem Paar stattfinden
;(every (lambda (firstEl) (every (lambda (secondEl)
;Sonst genügt es ein Paar zu finden, bei dem nicht gilt, dass (cdr firstEL) = (car secondEl)
;Zur Demonstration:
(define trans '((1 . 2) (2 . 3) (1 . 3)))
(define not-trans '((1 . 2) (2 . 3))) ;Fehler entsteht durch Vergleich (1 . 2) mit (1 . 2) auf
'trans
(transitiv? trans)
(transitiv? not-trans)
;;;;;
...
#|
(a) (+ (* 3 4) (* 5 6))
(b) (sqrt (- 1 (sin (sin x)))) ;;;;;Zweimal sin?
|#
...
(b) (define (mytan a) (/ (sin a) (sqrt (- 1 (sin (sin x))))))
                                            ;;;;; x ist undefiniert, der Parameter heißt a
|#
